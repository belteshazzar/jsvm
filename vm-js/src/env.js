...
export function createDefaultEnv({ onPrint, onUnhandledRejection } = {}) {
  ...
  StringProto.map.slice = {
    type: 'native', name: 'String.prototype.slice', arity: 2,
    call: (vm, args, thisObj) => {
      const s = stringValue(thisObj);
      const len = s.length;
      const toInt = (v, defaultValue) => {
        if (!v || v.type === 'null' || v.type === 'undef') return defaultValue;
        if (v.type !== 'num') panic('String.slice indices must be numbers');
        return v.value | 0;
      };
      let start = toInt(args[0], 0);
      let end = toInt(args[1], len);
      if (start < 0) start = Math.max(0, len + start);
      if (end < 0) end = Math.max(0, len + end);
      start = Math.min(Math.max(0, start), len);
      end = Math.min(Math.max(0, end), len);
      return { type: 'str', value: s.slice(start, end) };
    }
  };
  StringProto.map.indexOf = {
    type: 'native', name: 'String.prototype.indexOf', arity: 2,
    call: (vm, args, thisObj) => {
      const s = stringValue(thisObj);
      const search = (args[0] && args[0].type === 'str') ? args[0].value : vm.toStringV(args[0] ?? { type: 'undef' });
      const from = (args[1] && args[1].type === 'num') ? args[1].value | 0 : 0;
      return { type: 'num', value: s.indexOf(search, Math.max(0, from)) };
    }
  };
  StringProto.map.includes = {
    type: 'native', name: 'String.prototype.includes', arity: 2,
    call: (vm, args, thisObj) => {
      const s = stringValue(thisObj);
      const search = (args[0] && args[0].type === 'str') ? args[0].value : vm.toStringV(args[0] ?? { type: 'undef' });
      const from = (args[1] && args[1].type === 'num') ? args[1].value | 0 : 0;
      return { type: 'bool', value: s.includes(search, Math.max(0, from)) };
    }
  };
