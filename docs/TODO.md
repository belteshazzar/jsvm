...
- [x] String methods (curated): `slice`, `includes`, `indexOf`, `toUpperCase`, … — tests: `test/features/strings.methods.test.js`
- [x] Array method expansions (currently includes `push`, `pop`, `slice`, `indexOf`, `includes`, `join`, `shift`, `unshift`, `splice`, `reverse`, `sort` (no comparator callback), `concat`) — tests: `test/features/arrays.methods.test.js`
